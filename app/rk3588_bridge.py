...
class RK3588VectorBridge:
    def __init__(self, config=None, log=None, on_ready=None):
    ...
    @property
    def target_input_path(self):
        return self._abs_path(self._config.get('target_input_path', ''))
    ...
    def start(self):
        with self._proc_lock:
            if not self.enabled or self.running:
                return
            if not self.check():
                self._last_error = 'RK3588 运行时路径不完整'
            ...
            try:
                ...
                self._stdout_thread = threading.Thread(target=self._consume_pipe, args=(self._proc.stdout, 'RK3588'), daemon=True)
                self._stderr_thread = threading.Thread(target=self._consume_pipe, args=(self._proc.stderr, 'RK3588 ERR'), daemon=True)
                self._stdout_thread.start()
                self._stderr_thread.start()
                threading.Thread(target=self._watch_ready, daemon=True).start()
                self._log('🚀 RK3588 向量运行时已启动')
            except Exception as exc:
                self._last_error = str(exc)
                self._log(f'❌ RK3588 向量运行时启动失败: {exc}')
            ...
            if proc.poll() is None:
                ...
                try:
                    ...
                    try:
                        ...
                        self._last_error = 'RK3588 向量运行时无法终止，可能卡在内核/摄像头驱动'
                        self._log(f'⚠️ {self._last_error}')
                        return False
            self._proc = None
            self._ready = False
    ...
    def restart(self, reason=''):
        with self._proc_lock:
            if reason:
                ...
                self._log(f'🔁 RK3588 向量运行时重启: {reason}')
            if not self.stop():
                return False
            self.start()
            return True

    def set_target_class(self, target_class_name):
